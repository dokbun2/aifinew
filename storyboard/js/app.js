...
function createTestData() {
   ...
   try {
       if (!currentData || !currentData.breakdown_data || !currentData.breakdown_data.shots) {
           return showMessage('저장할 URL 데이터가 없습니다.', 'error');
       }
       ...
       currentData.breakdown_data.shots.forEach(shot => {
           ...
           if (shot.image_design?.ai_generated_images) {
               shotUrls.image_urls = shot.image_design.ai_generated_images;
           }
           
           // 비디오 URL
           if (shot.video_urls) {
               shotUrls.video_urls = shot.video_urls;
           }
           
           // 오디오 URL
           if (shot.content?.audio_urls) {
               shotUrls.audio_urls = shot.content.audio_urls;
           }
           
           // 참조 이미지 URL
           if (shot.reference_images) {
               shotUrls.reference_images = shot.reference_images;
           }
           
           if (Object.keys(shotUrls).length > 0) {
               urlData.urls[shot.id] = shotUrls;
           }
       });
       
       // 프로젝트 레벨 음악 URL
       if (currentData.film_metadata?.project_music_urls) {
           urlData.project_music_urls = currentData.film_metadata.project_music_urls;
       }
       
       const jsonFileName = getProjectFileName().replace('.json', '_urls.json');
       ...
       showMessage(`URL 데이터가 ${jsonFileName} 파일로 저장되었습니다.`, 'success');
   } catch (error) {
       showMessage(`URL 내보내기 오류: ${error.message}`, 'error');
   }
       }
...
try {
    ...
    window.location.href = '../your_title_storyboard_v9.4_c.html';
